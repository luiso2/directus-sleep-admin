...
export const CustomerEdit: React.FC = () => {
  ...
  const { formProps, saveButtonProps, query } = useForm<Customer>({
    resource: "customers",
    redirect: "show",
    onMutationSuccess: () => {
      navigate("/customers");
  ...
  return (
    <Edit 
      ...
      title={`Editar Cliente: ${customer?.first_name} ${customer?.last_name}`}
    >
              ...
              <Card title="Información del Sistema" bordered={false} size="small">
                <Row gutter={16}>
                  <Col span={12}>
                    ...
                    <p><strong>Creado:</strong> {customer.created_at ? new Date(customer.created_at).toLocaleString("es-MX") : "No disponible"}</p>
                  </Col>
                  <Col span={12}>
                    <p><strong>Última actualización:</strong> {customer.updated_at ? new Date(customer.updated_at).toLocaleString("es-MX") : "No disponible"}</p>
                  </Col>
                </Row>
              </Card>
            </Col>
