...
import type { CreateSaleForm, Customer } from "../../types/directus";
...
export const SaleCreate: React.FC = () => {
  ...
  const { formProps, saveButtonProps, form } = useForm<CreateSaleForm>({
    resource: "sales",
    redirect: "show",
    onMutationSuccess: () => {
      navigate("/sales");
  ...
  useEffect(() => {
    const date = new Date();
    const saleNumber = `VNT-${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}-${String(date.getHours()).padStart(2, '0')}${String(date.getMinutes()).padStart(2, '0')}`;
    form.setFieldsValue({ sale_number: saleNumber });
  }, [form]);
  ...
  return (
    <Create saveButtonProps={saveButtonProps} title="Nueva Venta">
